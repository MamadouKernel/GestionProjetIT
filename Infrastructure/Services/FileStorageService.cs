...
namespace GestionProjects.Infrastructure.Services
{
    ...
    public class FileStorageService : IFileStorageService
    {
        ...
        public async Task<string> SaveGeneratedFileAsync(byte[] content, string fileName, string subfolder, string? identifier = null)
        {
            if (content == null || content.Length == 0)
                throw new ArgumentException("Le contenu du fichier généré est vide.");

            if (string.IsNullOrWhiteSpace(fileName))
                throw new ArgumentException("Le nom du fichier généré est requis.");
            ...
            var safeBaseName = Path.GetFileNameWithoutExtension(fileName);
            var extension = Path.GetExtension(fileName);
            var safeFileName = $"{safeBaseName}_{Guid.NewGuid():N}{extension}";
            var filePath = Path.Combine(uploadsPath, safeFileName);
        ...
        public string GetAbsolutePath(string relativePath)
        {
            if (string.IsNullOrWhiteSpace(relativePath))
                throw new ArgumentException("Le chemin du fichier est requis.", nameof(relativePath));
            ...
            {
                throw new UnauthorizedAccessException("Chemin de fichier invalide.");
            }

            var absolutePath = Path.GetFullPath(Path.Combine(_environment.WebRootPath, normalizedRelativePath));
            var absoluteWebRoot = Path.GetFullPath(_environment.WebRootPath);

            if (!absolutePath.StartsWith(absoluteWebRoot, StringComparison.OrdinalIgnoreCase))
                throw new UnauthorizedAccessException("Accès au fichier non autorisé.");
        ...
        public bool ValidateFileSignature(UploadedFileInput file, string[] allowedExtensions)
        {
            ...
            {
                // Aucune signature magic bytes connue pour cette extension → refus par défaut
                return false;
            }
