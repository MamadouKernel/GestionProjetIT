...
namespace GestionProjects.Infrastructure.Services
{
    ...
    {
        private static Paragraph? CreateLogoParagraph(MainDocumentPart mainPart)
        {
            ...
            const long widthEmus = 1100000L;
            const long heightEmus = 900000L;
            ...
            var drawing = new Drawing(
                new DW.Inline(
                    ...
                    new DW.DocProperties { Id = (UInt32Value)1U, Name = "Logo CIT" },
                    new DW.NonVisualGraphicFrameDrawingProperties(new A.GraphicFrameLocks { NoChangeAspect = true }),
                    new A.Graphic(
                        new A.GraphicData(
                            new PIC.Picture(
                                new PIC.NonVisualPictureProperties(
                                    new PIC.NonVisualDrawingProperties { Id = (UInt32Value)0U, Name = "Logo CIT" },
            ...
            return await Task.Run(() =>
            {
                using (var stream = new MemoryStream())
                {
                    using (var wordDocument = WordprocessingDocument.Create(stream, WordprocessingDocumentType.Document))
                    {
                        ...
                        // En-tête
                        var logoParagraph = CreateLogoParagraph(mainPart);
                        if (logoParagraph != null)
                        {
                            body.AppendChild(logoParagraph);
                        }
                        body.AppendChild(CreateParagraph(DocumentBrandingHelper.CompanyName.ToUpperInvariant(), true, 14, "center"));
                        body.AppendChild(CreateParagraph($"{DocumentBrandingHelper.CompanySite} - {DocumentBrandingHelper.ApplicationName}", false, 10, "center"));
