...
namespace GestionProjects.Controllers
{
    ...
    public class DemandeProjetController : Controller
    {
        ...
        public async Task<IActionResult> Details(Guid id)
        {
            ...
            // IDOR: un Demandeur ne peut voir que ses propres demandes
            ...
            var canManageDemandes = await CanManageDemandesBackofficeAsync();
            var canActAsDemandeur = await _permissionService.CurrentUserHasPermissionAsync("DemandeProjet", "Details") &&
                                    demande.DemandeurId == userId;
            var canEditAsDemandeur = await _permissionService.CurrentUserHasPermissionAsync("DemandeProjet", "Edit") &&
                                     demande.DemandeurId == userId;
            var canSubmitDemande = await _permissionService.CurrentUserHasPermissionAsync("DemandeProjet", "Soumettre") &&
                                   demande.DemandeurId == userId;
            var canAddDocuments = await _permissionService.CurrentUserHasPermissionAsync("DemandeProjet", "AjouterDocumentsComplementaires") &&
                                  demande.DemandeurId == userId;
            var canDuplicateDemande = await _permissionService.CurrentUserHasPermissionAsync("DemandeProjet", "DupliquerDemande") &&
                                      demande.DemandeurId == userId;
            var canActAsDm = await _permissionService.CurrentUserHasPermissionAsync("DemandeProjet", "ValiderDM") &&
                             demande.DirecteurMetierId == userId;
        ...
        public async Task<IActionResult> Create(DemandeProjet demande, IFormFile? cahierCharges, List<IFormFile>? annexes)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // Neutraliser les champs système qui ne doivent pas venir du formulaire
                ...
                // Notification Teams + Email : nouvelle demande
...
Risques de sécurité liés aux échanges de données sensibles: Atténué par l'implémentation de solutions sécurisées et conformes aux politiques de sécurité",
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            var userId = User.GetUserIdOrThrow();
            var canManageDemandes = await CanManageDemandesBackofficeAsync();
            var canEditOwnDemande = await _permissionService.CurrentUserHasPermissionAsync("DemandeProjet", "Edit") &&
                                    demande.DemandeurId == userId;
            var canEditAsDm = await _permissionService.CurrentUserHasPermissionAsync("DemandeProjet", "ValiderDM") &&
                              demande.DirecteurMetierId == userId;
            ...
            List<IFormFile>? annexes)
        {
            ...
            var canManageDemandes = await CanManageDemandesBackofficeAsync();
            var canEditOwnDemande = await _permissionService.CurrentUserHasPermissionAsync("DemandeProjet", "Edit") &&
                                    existingDemande.DemandeurId == userId;
            var canEditAsDm = await _permissionService.CurrentUserHasPermissionAsync("DemandeProjet", "ValiderDM") &&
                              existingDemande.DirecteurMetierId == userId;
            if (!canManageDemandes && !canEditOwnDemande && !canEditAsDm)
                return Forbid();
            ...
            string? titre, string? description, string? objectifs, string? avantagesAttendus)
        {
            ...
            // Empêcher l'auto-validation : le demandeur ne peut pas valider sa propre demande
            ...
            // Capturer les valeurs AVANT modification pour l'historique
            var avant = new {
                Titre = demande.Titre,
                Description = demande.Description,
                Objectifs = demande.Objectifs,
                AvantagesAttendus = demande.AvantagesAttendus,
                StatutDemande = ancienStatut.ToString()
            };
            ...
            // Log enrichi : avant/après avec diff des champs modifiés
            var apres = new {
                Titre = demande.Titre,
                Description = demande.Description,
                Objectifs = demande.Objectifs,
                AvantagesAttendus = demande.AvantagesAttendus,
                StatutDemande = demande.StatutDemande.ToString(),
...
Risques de sécurité liés aux échanges de données sensibles: Atténué par l'implémentation de solutions sécurisées et conformes aux politiques de sécurité",
            ...
            await _auditService.LogActionAsync("CreationProjet", "Projet", projet.Id,
                null,
            ...
            _ = _email.EnvoyerValidationDSIProjetCreeAsync(
                demandeurDSI?.Email ?? string.Empty,
                dmDSI?.Email,
                demande.Titre ?? string.Empty,
                projet.CodeProjet);
        ...
        public async Task<IActionResult> RejeterDSI(Guid id, string? commentaire)
        {
            ...
            _ = _email.EnvoyerRejetDSIAsync(
                demandeurRejetDSI?.Email ?? string.Empty,
                dmRejetDSI?.Email,
        ...
        public async Task<IActionResult> HistoriqueActionsDM(int page = 1, int pageSize = 20)
        {
            ...
            // Charger les demandes correspondantes
            var demandeIds = logs
