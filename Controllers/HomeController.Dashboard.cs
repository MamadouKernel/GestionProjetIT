        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (isDsiOrAdmin)
            {
                ...
                stats.CloturesEnAttente = demandesCloture.Count(dc =>
                    !dc.EstTerminee &&
                    dc.StatutValidationDemandeur == Domain.Enums.StatutValidationCloture.Validee &&
                    dc.StatutValidationDirecteurMetier == Domain.Enums.StatutValidationCloture.Validee &&
                    dc.StatutValidationDSI == Domain.Enums.StatutValidationCloture.EnAttente);
                ...
                stats.RisquesCritiques = risques.Count(r => r.Impact == Domain.Enums.ImpactRisque.Critique);
                ...
                stats.AnomaliesOuvertes = anomalies.Count(a => a.Statut == Domain.Enums.StatutAnomalie.Ouverte);
                ...
                var risqueParProjet = risques
                    ...
                    .ToDictionary(
                        ...
                        g => new
                        {
                            ...
                            Eleves = g.Count(r => r.Impact == Domain.Enums.ImpactRisque.Eleve),
                            MaxImpact = g.Max(r => r.Impact)
                        });

                var anomaliesOuvertesParProjet = anomalies
                    .Where(a => a.Statut == Domain.Enums.StatutAnomalie.Ouverte)
                ...
                stats.AdminCriticalProjects = projets
                    ...
                    .Select(p =>
                    {
                        ...
                        return new DashboardCriticalProjectItem
                        {
                            ...
                            Url = Url.Action("Details", "Projet", new { id = p.Id }) ?? "#"
                        };
                    })
                    .ToList();
                ...
                validationRows.AddRange(demandesValidationDM.Select(d => (
                    ...
                    new DashboardValidationItem
                    {
                        ...
                        Url = Url.Action("Details", "DemandeProjet", new { id = d.Id }) ?? "#"
                    })));
                ...
                validationRows.AddRange(projets
                    ...
                    .Select(p => (
                        ...
                        new DashboardValidationItem
                        {
                            ...
                            Url = Url.Action("Details", "Projet", new { id = p.Id, tab = "analyse" }) ?? "#"
                        })));
                ...
                validationRows.AddRange(projets
                    ...
                    .Select(p => (
                        ...
                        new DashboardValidationItem
                        {
                            ...
                            Demandeur = FormatUtilisateurCourt(p.DemandeProjet?.Demandeur?.Nom, p.DemandeProjet?.Demandeur?.Prenoms),
                            DateLabel = (p.DateModification ?? p.DateCreation).ToString("dd/MM/yyyy"),
                            Url = Url.Action("Details", "Projet", new { id = p.Id, tab = "planification" }) ?? "#"
                        })));
                ...
                validationRows.AddRange(demandesCloture
                    ...
                    .Select(dc => (
                        ...
                        new DashboardValidationItem
                        {
                            ...
                            Url = Url.Action("Details", "Projet", new { id = dc.ProjetId, tab = "cloture" }) ?? "#"
                        })));
                ...
                alertRows.AddRange(risqueParProjet
                    ...
                    .Select(r =>
                    {
                        ...
                        return new DashboardAlertItem
                        {
                            ...
                            Url = projet != null ? (Url.Action("Details", "Projet", new { id = projet.Id, tab = "analyse" }) ?? "#") : "#"
                        };
                    }));
                ...
                alertRows.AddRange(anomaliesOuvertesParProjet
                    ...
                    .Select(a =>
                    {
                        ...
                        return new DashboardAlertItem
                        {
                            ...
                            DateLabel = now.ToString("dd/MM/yyyy HH:mm"),
                            Url = projet != null ? (Url.Action("Details", "Projet", new { id = projet.Id, tab = "uat" }) ?? "#") : "#"
                        };
                    }));
            ...
            else
            {
                ...
                if (isRsi)
                {
                    ...
                    stats.RisquesCritiques = await _db.Set<Domain.Models.RisqueProjet>().CountAsync(r => r.Impact == Domain.Enums.ImpactRisque.Critique);
                    stats.TotalAnomalies = await _db.Set<Domain.Models.AnomalieProjet>().CountAsync();
                    stats.AnomaliesOuvertes = await _db.Set<Domain.Models.AnomalieProjet>().CountAsync(a => a.Statut == Domain.Enums.StatutAnomalie.Ouverte);
                ...
                if (isDm)
                {
                    ...
                    IQueryable<Domain.Models.Projet> projetsQueryDM = _db.Projets.Where(p => p.SponsorId == userId);
                ...
                if (isCp)
                {
                    ...
                    stats.TotalRisques = await _db.Set<Domain.Models.RisqueProjet>().CountAsync(r => r.Projet!.ChefProjetId == userId);
                    stats.RisquesCritiques = await _db.Set<Domain.Models.RisqueProjet>().CountAsync(r => r.Projet!.ChefProjetId == userId && r.Impact == Domain.Enums.ImpactRisque.Critique);
                    stats.AnomaliesOuvertes = await _db.Set<Domain.Models.AnomalieProjet>().CountAsync(a => a.Projet!.ChefProjetId == userId && a.Statut == Domain.Enums.StatutAnomalie.Ouverte);
                    stats.TotalLivrables = await _db.Set<Domain.Models.LivrableProjet>().CountAsync(l => l.Projet!.ChefProjetId == userId);
